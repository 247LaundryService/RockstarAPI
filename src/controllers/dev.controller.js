...
const sendToMRIValidate = async (req, _code) => {
  ...
  const secret = process.env.MRI_SECRET_KEY_DEV;
  ...
  const ret = await axios
    .post(process.env.MRI_VALIDATE_URL_DEV, body, { headers })
    .then((response) => {
...
const sendToMRISubmit = async (req, _code) => {
  ...
  const ret = await axios
    .post(process.env.MRI_REGISTER_URL_DEV, body, { headers })
    .then((response) => {
      console.log(response.data);
...
const sendToMRIReset = async (req) => {
  const CurrentDate = new Date(Date.now()).toISOString();

  console.log('==>>> CurrentDate: ', CurrentDate, '\n');

  // Create string for hashing
  const stringForsignature = `${process.env.MRI_PROGRAM_ID}${CurrentDate}`;
  console.log('==>>> stringForsignature: ', stringForsignature, '\n');
  ...
  const ret = await axios
    .post(process.env.MRI_RESET_URL_DEV, {}, { headers })
    .then((response) => {
...
const sendToSF = async (req) => {
  ...
  const ret = await axios
    .post(process.env.SF_SUBMIT_URL_DEV, bodySubmit, {
 ...
 */
 exports.postReset = async (req, res, next) => {
  const ret = await sendToMRIReset(req);
  responseManager(res, ret.data, ret.success, ret.message);
